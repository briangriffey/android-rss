...
class RSSHandler extends org.xml.sax.helpers.DefaultHandler {
    ...
    /**
     * Setter for the enclosure tag that can hold such things as mp3 streams
     * @author briangriffey
     */
    private final Setter ENCLOSURE = new AttributeSetter() {
        ...
        public void set(Attributes attributes) {
            ...
            final int length = MediaAttributes.intValue(attributes, LENGTH, 0);

            if(url == null)
                return;

            MediaEnclosure enclosure = new MediaEnclosure(url, length, mimeType);
